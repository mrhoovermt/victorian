...
export default function CateringPage() {
  ...
  const [selected, setSelected] = useState<CateringLead | null>(null);
  ...
  async function handleSave(id: string, updates: Partial<CateringLead>) {
    await fetch(`/api/admin/catering/${id}`, {
      method: "PATCH",
  ...
  return (
    <div>
      <h1 className="text-2xl text-white font-light mb-6">Catering Leads <span className="text-gray-600 text-lg">({total})</span></h1>
        ...
        <table className="w-full min-w-[700px]">
          <thead>
            <tr className="border-b border-[#2a2a2a]">
              {["Name","Email","Phone","Event Type","Event Date","Headcount","Contacted","Received"].map((h) => (
          ...
          <tbody>
            {leads.map((l) => (
              <tr key={l.id} className="border-b border-[#1a1a1a] hover:bg-[#111] cursor-pointer transition-colors" onClick={() => setSelected(l)}>
                ...
                <td className="px-3 py-3 text-gray-400 text-sm font-sans">{l.eventType ?? "—"}</td>
                <td className="px-3 py-3 text-gray-400 text-sm font-sans whitespace-nowrap">{l.eventDate ? new Date(l.eventDate).toLocaleDateString() : "—"}</td>
                <td className="px-3 py-3 text-gray-400 text-sm font-sans">{l.headcount ?? "—"}</td>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-start justify-end z-50">
          <div className="bg-[#0d0d0d] border-l border-[#2a2a2a] h-full w-full max-w-md overflow-y-auto p-8">
            ...
            <div className="space-y-3 mb-6 text-sm font-sans">
              ...
              <p><span className="text-gray-500">Event:</span> <span className="text-gray-300">{selected.eventType ?? "—"}</span></p>
              <p><span className="text-gray-500">Date:</span> <span className="text-gray-300">{selected.eventDate ? new Date(selected.eventDate).toLocaleDateString() : "—"}</span></p>
              <p><span className="text-gray-500">Headcount:</span> <span className="text-gray-300">{selected.headcount ?? "—"}</span></p>
            ...
            <div className="space-y-3">
              <div>
                ...
                <textarea
                  rows={4}
                  className="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white px-3 py-2 text-sm font-sans focus:outline-none focus:border-[#c9a96e]"
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  defaultChecked={selected.contacted}
                  id="contacted"
                />
              ...
              <button
                onClick={() => {
                  const notes = (document.getElementById("internalNotes") as HTMLTextAreaElement)?.value;
                  const contacted = (document.getElementById("contacted") as HTMLInputElement)?.checked;
